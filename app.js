...
const LS = {
  ...
  supabaseUrl: 'https://rpgueqafknvrwvrbzixd.supabase.co',
  supabaseKey: 'sb_publishable_TsTrCMyNceS2QH3b2MbpEg_A6MRkade',
  prefs: 'indybooks_prefs',
  sleepTimer: 'indybooks_sleep_until',
  installDismissed: 'indybooks_install_dismissed',
};
...
const Cloud = window.Cloud || (() => {
  ...
  return {
    ...
    canFetchFeeds: () => false,
    fetchFeed: () => Promise.reject(new Error('Feed import needs cloud sync.')),
    maxUploadBytes: () => 0,
  };
})();
...
function onAudioTimeUpdate() {
  ...
  // Cheap deadline check — catches a timer that was throttled while the
  // screen was off, without waiting for a visibilitychange.
  if (runtime.sleepTimerEndsAt && checkSleepTimer()) return;
...
async function ingestRssFeed() {
  ...
  if (!Cloud.canFetchFeeds()) {
    if (button) { button.disabled = false; button.textContent = original; }
    toast('Sign in to import feeds — fetching happens on the server.', 'error');
  ...
  try {
    // Fetched by our own Edge Function rather than a public CORS proxy, so
    // feed URLs never leave the project and there is no third-party
    // dependency in the import path. See supabase/functions/fetch-feed.
    const data = await Cloud.fetchFeed(feedUrl, controller.signal);
    ...
    toast(err.name === 'AbortError'
      ...
      // The Edge Function returns a specific reason (bad URL, blocked host,
      // upstream status), which is far more actionable than a generic message.
      : (err.message || 'That feed could not be read. Check the URL and try again.'),
      'error');
  } finally {
 ...
 * Sleep timer.
 *
 * setTimeout alone is not trustworthy here. Once the app is backgrounded or
 * the tab is suspended — which is the normal state for a sleep timer, since
 * the user has put the phone down — browsers throttle or defer timers, and on
 * iOS a suspended web app may not run them at all. The original 60-minute
 * timer could simply never fire.
 *
 * So the deadline is the source of truth, not the timer. It is persisted, the
 * timer is only a best-effort nudge, and the deadline is re-checked whenever
 * the app becomes visible again or audio ticks. Whichever happens first wins.
 */
...
function armSleepTimer() {
  ...
  if (!runtime.sleepTimerEndsAt) return;
  const remaining = runtime.sleepTimerEndsAt - Date.now();
  runtime.sleepTimerTimeout = setTimeout(checkSleepTimer, Math.max(0, remaining));
  runtime.sleepTimerInterval = setInterval(updateSleepTimerLabel, 1000);
}

/** The only place playback actually stops. Safe to call as often as you like. */
function checkSleepTimer() {
  if (!runtime.sleepTimerEndsAt) return false;
  if (Date.now() < runtime.sleepTimerEndsAt) { updateSleepTimerLabel(); return false; }
...
function persistSleepTimer() {
  try {
    if (runtime.sleepTimerEndsAt) {
      localStorage.setItem(LS.sleepTimer, String(runtime.sleepTimerEndsAt));
    } else {
      localStorage.removeItem(LS.sleepTimer);
    }
  } catch { /* storage full; the in-memory deadline still works this session */ }
}

/**
 * Restore a timer that was running when the app was closed. If the deadline
 * has already passed we do not resume playback, so the user does not come
 * back to a phone that started reading to an empty room.
 */
function restoreSleepTimer() {
  let saved = null;
  try { saved = localStorage.getItem(LS.sleepTimer); } catch { /* ignore */ }
  if (!saved) return;
  const endsAt = Number(saved);
  if (!Number.isFinite(endsAt) || endsAt <= Date.now()) {
    try { localStorage.removeItem(LS.sleepTimer); } catch { /* ignore */ }
    return;
  }
  runtime.sleepTimerEndsAt = endsAt;
  armSleepTimer();
  updateSleepTimerLabel();
}
...
const actions = {
  ...
  'open-settings': () => { openModal('settings-modal'); refreshStorageReadout(); },
  ...
  'install': runInstall,
  'dismiss-install': () => dismissInstall(false),
  'install-from-settings': installFromSettings,
...
function bindEvents() {
  ...
  document.addEventListener('visibilitychange', () => {
    ...
    // Coming back to the foreground: the deadline may have passed while
    // timers were frozen, and the pending timeout may be badly stale.
    if (runtime.sleepTimerEndsAt && !checkSleepTimer()) armSleepTimer();
  });
...
/* ------------------------------------------------------------------ *
 * 20. Install & storage durability
 *
 * With no app store in the distribution path, these two things carry the
 * weight a store listing normally would: getting the app onto the home
 * screen, and keeping its data there.
 * ------------------------------------------------------------------ */
...
/** Chrome-family in-app browsers hide Add to Home Screen entirely. */
function isIosInAppBrowser() {
  return isIos() && /FBAN|FBAV|Instagram|Line|Twitter|CriOS|FxiOS|EdgiOS/i.test(navigator.userAgent);
}

function initInstall() {
  // Android/desktop Chromium: capture the real prompt and show our own button.
  window.addEventListener('beforeinstallprompt', (e) => {
    e.preventDefault();
    deferredInstallPrompt = e;
    if (!isStandalone() && !installDismissed()) showInstallCard('prompt');
  });

  window.addEventListener('appinstalled', () => {
    deferredInstallPrompt = null;
    hideInstallCard();
    toast('Installed. Open IndyBooks from your home screen.', 'success');
  });

  // Safari implements no beforeinstallprompt at all, so on iOS the guidance
  // below IS the install flow. Delayed slightly so it does not compete with
  // first paint.
  if (isIos() && !isStandalone() && !installDismissed()) {
    setTimeout(() => showInstallCard(isIosInAppBrowser() ? 'ios-browser' : 'ios'), 1200);
  }
}

function installDismissed() {
  try { return localStorage.getItem(LS.installDismissed) === '1'; }
  catch { return false; }
}
...
function showInstallCard(mode) {
  ...
  if (!card || !body || !action) return;

  if (mode === 'prompt') {
    body.innerHTML = 'Install IndyBooks to get its own icon, full screen, and offline playback.';
    action.textContent = 'Install';
    action.classList.remove('hidden');
  } else if (mode === 'ios') {
    // Naming the exact taps matters: there is no prompt to fall back on.
    body.innerHTML = 'Add IndyBooks to your home screen: tap '
      + '<strong><i class="fa-solid fa-arrow-up-from-bracket" aria-hidden="true"></i> Share</strong>'
      + ' at the bottom of Safari, then <strong>Add to Home Screen</strong>.';
    action.classList.add('hidden');
  } else {
    body.textContent = 'Open this page in Safari to add IndyBooks to your home screen. '
      + 'This in-app browser cannot install it.';
    action.classList.add('hidden');
  }
...
async function runInstall() {
  if (!deferredInstallPrompt) { hideInstallCard(); return; }
  ...
  try {
    ...
    if (outcome === 'dismissed') dismissInstall(true);
  } catch (err) {
...
function dismissInstall(silent = false) {
  hideInstallCard();
  try { localStorage.setItem(LS.installDismissed, '1'); } catch { /* ignore */ }
  if (!silent) toast('You can install later from Settings.');
}

/** Re-offer from Settings even after the banner was dismissed. */
function installFromSettings() {
  try { localStorage.removeItem(LS.installDismissed); } catch { /* ignore */ }
  closeModals();
  if (isStandalone()) { toast('IndyBooks is already installed.'); return; }
  if (deferredInstallPrompt) { showInstallCard('prompt'); return; }
  showInstallCard(isIos() ? (isIosInAppBrowser() ? 'ios-browser' : 'ios') : 'prompt');
}

/**
 * Ask the browser to protect our storage from eviction.
 *
 * This is the single most important call for an offline audiobook app with no
 * store presence. Browsers evict "best-effort" origins under storage
 * pressure, and Safari has historically capped script-writable storage for
 * sites that go unused. Downloaded audiobooks are exactly the kind of large,
 * infrequently-touched data that gets reclaimed first.
 *
 * Safari requires this to be re-requested on each launch, so it runs every
 * time rather than once. It is also not a guarantee on any engine — hence the
 * cloud sync, which means an eviction costs downloads rather than the library.
 */
async function initPersistentStorage() {
  ...
  try {
    ...
    if (!granted) {
      console.info('Persistent storage not granted; downloads may be evicted '
        + 'under storage pressure. Installing to the home screen improves the odds.');
    }
  } catch (err) {
...
/**
 * Show what the app is using and whether that data is protected. Audio files
 * are large enough that users need to see this before they wonder where their
 * disk went.
 */
async function refreshStorageReadout() {
  const el = document.getElementById('storage-readout');
  ...
  try {
    ...
    el.innerHTML = `Using <strong>${formatBytes(usage)}</strong>`
      + (quota ? ` of about ${formatBytes(quota)} available` : '')
      + `. Downloads are ${persisted
        ? '<strong>protected</strong> from automatic cleanup.'
        : 'not protected yet — installing to your home screen helps.'}`;
  } catch (err) {
