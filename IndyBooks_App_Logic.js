...
function combineSelectedItems() {
    ...
    const selectedItems = appState.items.filter(i => appState.selectedItemIds.includes(i.id));
    const combinedItem = {
        ...
        subTracks: selectedItems.map(i => ({ title: i.title, url: i.url })),
        currentTime: 0,
        duration: 0,
        speed: 1.0,
        bookmarks: []
    ...
    if (!appState.folders.includes('Volumes')) {
        ...
        appState.folderColors['Volumes'] = '#EB8861';
    }
    ...
    return `
        <div class="p-3 flex items-center justify-between hover:bg-gray-50/80 transition ${isCurrent ? 'bg-[#C3D7EA]/20 border-l-4 border-[#4173B0]' : ''}">
            <div class="flex items-center space-x-3 flex-1 min-w-0 cursor-pointer" onclick="playItem('${item.id}')">
                <input type="checkbox" ${isChecked ? 'checked' : ''} onclick="toggleItemSelection('${item.id}', event)" class="w-4 h-4 accent-[#4173B0] rounded cursor-pointer shrink-0">
